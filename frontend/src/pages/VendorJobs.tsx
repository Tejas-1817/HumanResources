  ...
  return (
    <div className="space-y-8">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl sm:text-3xl font-bold text-foreground mb-1">My Job Openings</h1>
          <p className="text-xs sm:text-sm text-muted-foreground">View and manage job roles explicitly assigned to you by HR.</p>
        </div>
      </div>
            ...
            >
              <div className="flex items-start justify-between gap-3">
                <div className="flex items-center gap-3 min-w-0">
                  <div className="w-10 h-10 rounded-xl bg-primary/10 flex items-center justify-center text-primary shrink-0 ring-1 ring-primary/10">
                ...
                {(() => {
                  ...
                  return (
                    <span className={`inline-flex px-2 py-0.5 rounded-full text-[9px] font-black uppercase tracking-widest border whitespace-nowrap shrink-0 ${cls}`}>
                      {label}
                    </span>
                  );
                })()}
              ...
              <div className="grid grid-cols-3 gap-2 pt-1">
                <div className="p-2 rounded-lg bg-secondary/40 text-center">
                  <p className="text-[9px] font-bold text-muted-foreground uppercase">Positions</p>
                  <p className="text-xs font-bold text-foreground mt-0.5">{job.positions_required || 1}</p>
                </div>
                <div className="p-2 rounded-lg bg-secondary/40 text-center">
                  <p className="text-[9px] font-bold text-muted-foreground uppercase">Experience</p>
                  <p className="text-xs font-bold text-foreground mt-0.5 truncate">{job.experience_required != null ? `${job.experience_required}y` : "Any"}</p>
                </div>
                <div className="p-2 rounded-lg bg-primary/5 border border-primary/10 text-center">
                  <p className="text-[9px] font-bold text-primary uppercase">Sent</p>
                  <p className="text-xs font-bold text-primary mt-0.5">{resumesByJob[job.id] || 0}</p>
                </div>
              </div>
              ...
              <div className="pt-2 border-t border-border/40 flex justify-end">
                <Link
                  ...
                  className="w-full py-2.5 rounded-lg bg-primary text-primary-foreground hover:bg-primary/90 transition-all text-xs font-bold uppercase tracking-wider shadow-md shadow-primary/20 flex items-center justify-center gap-2"
                >
      ...
      <div className="hidden md:block glass-card overflow-hidden">
        ...
        <div className="grid grid-cols-[3.2fr_1.3fr_1.3fr_1.3fr_1.1fr_1.6fr] gap-4 px-8 py-4 border-b border-border bg-secondary/30 text-[10px] font-black text-muted-foreground uppercase tracking-[0.2em] items-center">
          ...
          {jobs.length > 0 ? (
            jobs.map((job) => (
              <div 
                key={job.id} 
                onClick={() => navigate(`/vendor/jobs/${job.id}`)}
                className="group px-8 py-4 grid grid-cols-[3.2fr_1.3fr_1.3fr_1.3fr_1.1fr_1.6fr] gap-4 items-center hover:bg-primary/[0.03] transition-all cursor-pointer"
              >
                {/* 1. Job Role & Info */}
                ...
                <div className="flex justify-center items-center">
                  <Link
                    to={`/vendor/upload?jobId=${job.id}`}
                    onClick={(e) => e.stopPropagation()}
                    className="px-4 py-2 rounded-lg bg-primary text-primary-foreground hover:bg-primary/90 transition-all text-[11px] font-black uppercase tracking-widest shadow-lg shadow-primary/20 flex items-center justify-center gap-2 whitespace-nowrap"
                  >
                    <Plus className="w-3.5 h-3.5" />
                    Upload Resume
                  </Link>
