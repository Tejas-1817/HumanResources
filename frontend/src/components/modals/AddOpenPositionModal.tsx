...
import { getCompanies, createJobRole, createCompany } from "@/api/resumeiq";
...
// Updated validation schema based on requirements
const formSchema = z.object({
  company_id: z.string().min(1, "Client is required"),
  project_name: z.string().min(1, "Project Name is required"),
  title: z.string().min(3, "Position Name must be at least 3 characters"),
  skills: z.string().min(1, "Skills required is mandatory"),
  ...
  project_duration_val: z.string().min(1, "Project Duration is required"),
  project_duration_unit: z.string().min(1, "Duration Unit is required"),
  request_raised_by: z.string().min(1, "Request Raised By is required"),
  budget: z.string().min(1, "Budget is required"),
  description: z.string().min(1, "Responsibilities section is required"),
});
...
export const AddOpenPositionModal = ({ open, onClose }: AddOpenPositionModalProps) => {
  ...
  const mutation = useMutation({
    mutationFn: (values: FormValues) => {
      const budgetVal = values.budget;
      const raisedByEmp = EMPLOYEES.find(e => e.id === values.request_raised_by);
      const raisedByName = raisedByEmp ? `${raisedByEmp.name} (${raisedByEmp.designation})` : values.request_raised_by;
      const durationFormatted = `${values.project_duration_val} ${values.project_duration_unit}`;
      const projectName = values.project_name;

      // Serialize new fields into description
      const richDetails = `
=== REQUISITION METADATA ===
Project: ${projectName}
Project Start Date: ${values.project_start_date}
...
${values.description}`;
      ...
      return createJobRole({
        ...
        experience_required: values.experience_required ? parseFloat(values.experience_required) : null,
        project_time_period: durationFormatted,
      });
    },
    onSuccess: () => {
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
          ...
          {activePage === 1 && (
            <div className="space-y-5 animate-in fade-in slide-in-from-bottom-2 duration-200">
                  ...
                  {isAddingNewClient && (
                    <div className="mt-2 p-3 bg-secondary/30 rounded-xl border border-border/80 space-y-2">
                      ...
                      <div className="flex gap-2 justify-end">
                        ...
                        <button
                          type="button"
                          disabled={!newClientName || !newClientLocation}
                          onClick={() => addClientMutation.mutate({ name: newClientName, location: newClientLocation })}
              ...
              {/* Row 4: Experience, Mode & Location */}
